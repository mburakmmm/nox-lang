...
fn compileAndRun(allocator: std.mem.Allocator, source: []const u8) !std.process.RunResult {
    ...
    const ir = try nox.codegen.generateModule(allocator, module, checker_state.instantiations.items, generic_names.items, null, closure_infos, checker_state.defer_synthetic_names);
...
test "codegen: Faz M.8 — provably-safe metod çağrılarının ÜRETTİĞİ IR'da nox_exception_pending GERÇEKTEN YOK" {
    ...
    const ir = try nox.codegen.generateModule(allocator, module, &.{}, &.{}, null, .empty, .empty);
...
test "codegen: Faz GG.5 — döngü içinde AYNI str için TEK bir strlen çağrısı (deduplike edildi)" {
    ...
    // ÖNCE TEK bir `call $strlen` üretir, HER İKİ `s[i]` erişimi de AYNI
    // önceden-hesaplanmış temp'i YENİDEN KULLANIR — bu FONKSİYONUN ÜRETTİĞİ
    // IR'da `call $strlen` TAM OLARAK BİR KEZ görünmelidir.
    var count: usize = 0;
    var search_from: usize = 0;
    while (std.mem.indexOfPos(u8, ir, search_from, "call $strlen")) |pos| {
        count += 1;
        search_from = pos + 1;
    }
    // Elle `.ssa` dökümüyle DOĞRULANDI: `count_two` GG.5 ÖNCESİ 2 STATİK
    // `call $strlen` üretirdi (`s[i]`nin İKİ AYRI ifade konumu, `== "a"`/
    // `== "n"`) — GG.5 SONRASI TEK bir hoisted çağrıya (`@while_cond0`DAN
    // ÖNCE) DÜŞTÜ. `count_via_for`, `range(len(s))`in builtin `len()`
    // çağrısından (`genStrIndex`DEN BAĞIMSIZ, AYRI bir çağrı sitesi —
    // dokunulmadı) 1 VE döngü-içi TEK `s[i]`si İçin hoisted 1 = 2 (statik
    // sayı DEĞİŞMEZ, tek konum zaten dedupe EDİLECEK İKİNCİ bir konum YOK
    // — ama çalışma zamanında artık döngü GÖVDESİ İçinde DEĞİL). TOPLAM
    // (1 + 2) = **3** — GG.5 ÖNCESİ bu SAYI 4 OLURDU (`count_two`nun
    // dedupe EDİLMEMİŞ 2 statik konumu YÜZÜNDEN).
...
test "codegen: lowlevel arenasından bir değeri bloktan return etmek reddedilir (Unsupported)" {
    ...
    try std.testing.expectError(error.Unsupported, nox.codegen.generateModule(allocator, module, &.{}, &.{}, null, .empty, .empty));
}
...
test "codegen: Faz T.3 — debug_source_path VERİLİRSE dbgfile + doğru satır numaralı dbgloc üretilir" {
    ...
    const ir = try nox.codegen.generateModule(allocator, module, &.{}, &.{}, "fibonacci.nox", .empty, .empty);
