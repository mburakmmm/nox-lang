...
const TLS_READ_BUF: usize = TlsClient.min_buffer_len;
const TLS_WRITE_BUF: usize = TlsClient.min_buffer_len;
const SOCK_BUF: usize = TlsClient.min_buffer_len;
...
fn connectInner(conn: *TlsConn, host: []const u8, port: i64) !void {
    ...
    const now = std.Io.Timestamp.now(io, .real);
    conn.client = try TlsClient.init(&conn.stream_reader.interface, &conn.stream_writer.interface, .{
        .host = .{ .explicit = host },
        .ca = .{ .bundle = .{
