//! `std.crypto.tls.Client`in (Zig 0.16.0, `std/crypto/tls/Client.zig`)
//! FORKLANMIŞ+YAMALI kopyası — `runtime/stdlib_shims/tls.zig`/`smtp.zig`/
//! `websocket.zig`nin (nox.tls/nox.smtp/nox.websocket) ÜÇÜNÜN de ORTAK
//! olarak kullandığı TEK kaynak.
//!
//! **Forklama nedeni (GERÇEK bir tekrar-üretimle, smtp.gmail.com:465 VE
//! smtp.office365.com:587'ye karşı DOĞRULANDI):** orijinal `Client.zig`nin
//! `handshake_type` switch'i (bu dosyada ~satır 450-960) OPSİYONEL
//! `certificate_request` (tip 13) el sıkışma mesajı İçİn HİÇ bir `case`
//! İÇERMİYOR — bazı posta sunucuları (gmail/office365, RFC 8446 §4.3.2
//! gereği) BUNU EncryptedExtensions'tan SONRA, Certificate'tan ÖNCE
//! gönderiyor, bu YÜZDEN orijinal kod `error.TlsUnexpectedMessage` İLE
//! ÇÖKÜYORDU (sıradan HTTPS sunucuları — google.com/example.com — BUNU
//! GÖNDERMEDİĞİ İçİn ORADA sorun YOKTU). Upstream'de bilinen, HÂLÂ AÇIK
//! bir Zig hatası: `ziglang/zig#19521`.
//!
//! **Bu dosyada yapılan İKİ yama** (aşağıda `NOX PATCH` yorumlarıyla
//! işaretli, ikisi de SADECE TLS 1.3 İçİn — TLS 1.2'de `certificate_
//! request` YİNE İŞLENMİYOR, BİLİNÇLİ kapsam dışı, günümüz posta
//! sunucuları pratikte TLS 1.3 kullanıyor):
//! 1. Gelen `certificate_request` mesajını KABUL ET (çökmeden geç,
//!    `handshake_state` `.certificate`de KALIR — sunucunun KENDİ
//!    Certificate mesajı HÂLÂ bekleniyor).
//! 2. RFC 8446 §4.3.2 gereği, istemcinin KENDİ el sıkışma turunda BOŞ bir
//!    Certificate mesajı (istemci sertifikası YOK anlamında) GERİ
//!    gönder — AKSİ HALDE baz sunucular `Finished` doğrulamasını
//!    reddedebilir.
//!
//! **Zig yükseltme notu:** `build.zig` TAM Zig sürüm eşleşmesini zorunlu
//! kıldığından (`build.zig.zon`: `minimum_zig_version = "0.16.0"`),
//! gelecekteki bir Zig yükseltmesi ZATEN koordineli/manuel bir olay —
//! O ANDA bu dosya, yeni Zig sürümünün `std/crypto/tls/Client.zig`'i İLE
//! yeniden diff'lenip İKİ yama yeniden uygulanmalıdır.
//!
//! Tek bağımlılığı `std` (normal `@import("std")` İLE, orijinalin
//! `../../std.zig` göreli importu YERİNE) VE `builtin`dir — başka hiçbir
//! sibling-dosya bağımlılığı YOKTUR.
...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    // NOX PATCH (bkz. dosya başlığı): sunucunun opsiyonel `certificate_request`
    // mesajını kabul edip istemcinin kendi el sıkışma turunda boş bir
    // `Certificate` yanıtı inşa edebilmesi için gereken durum.
    var client_cert_requested = false;
    var cert_req_context_buf: [255]u8 = undefined;
    var cert_req_context_len: u8 = 0;
    var client_hs_write_seq: u64 = 0;
        ...
        switch (ct) {
            ...
            .handshake => while (true) {
                ...
                switch (handshake_type) {
                    ...
                    // NOX PATCH (bkz. dosya başlığı, ziglang/zig#19521): sunucunun
                    // opsiyonel `CertificateRequest`ini (RFC 8446 §4.3.2) kabul et.
                    // Sadece TLS 1.3'te, tam olarak sunucunun KENDİ Certificate
                    // mesajının beklendiği noktada geçerlidir — kabul ettikten
                    // sonra `handshake_state` KASITLI OLARAK `.certificate`de
                    // KALIR (sunucunun kendi Certificate mesajı hâlâ bekleniyor).
                    .certificate_request => {
                        if (tls_version != .tls_1_3) return error.TlsUnexpectedMessage;
                        if (cipher_state != .handshake) return error.TlsUnexpectedMessage;
                        ...
                        try hsd.ensure(1);
                        const ctx_len = hsd.decode(u8);
                        try hsd.ensure(ctx_len);
                        const ctx = hsd.slice(ctx_len);
                        @memcpy(cert_req_context_buf[0..ctx_len], ctx);
                        cert_req_context_len = ctx_len;
                        try hsd.ensure(2);
                        const ext_total = hsd.decode(u16);
                        _ = try hsd.sub(ext_total);
                        ...
                        const app_cipher = app_cipher: switch (handshake_cipher) {
                            inline else => |*p, tag| switch (tls_version) {
                                .tls_1_3 => {
                                    ...
                                    // NOX PATCH: application-traffic-secret transcript point (RFC 8446
                                    // §7.1 — stops at the server's Finished). `peek()` is NON-destructive
                                    // (unlike the original `finalResult()`), so `p.transcript_hash`
                                    // stays live for the optional client Certificate update below.
                                    const handshake_hash = p.transcript_hash.peek();

                                    // NOX PATCH (bkz. dosya başlığı, ziglang/zig#19521): sunucu
                                    // `certificate_request` gönderdiyse, RFC 8446 §4.3.2 gereği boş bir
                                    // istemci `Certificate` mesajı (sertifika YOK) inşa edip, `client_
                                    // handshake_key`yle AYRI bir şifreli kayıt olarak, Finished'tan ÖNCE
                                    // gönder. Transcript'e bu, Finished'ın KENDİ verify_data'sı
                                    // hesaplanmadan ÖNCE eklenmeli (Finished MAC'i istemcinin kendi
                                    // Certificate mesajını da kapsamalı) — AMA `handshake_hash` (yukarıda,
                                    // application-traffic-secret türetmesi İçin) BUNU İÇERMEMELİ.
                                    var cert_record_buf: [tls.record_header_len + (1 + 3 + 255 + 3 + 1) + P.AEAD.tag_length]u8 = undefined;
                                    var cert_record_len: usize = 0;
                                    if (client_cert_requested) {
                                        var cert_wrapped_buf: [1 + 3 + 255 + 3]u8 = undefined;
                                        const cert_body_len: u32 = 1 + @as(u32, cert_req_context_len) + 3;
                                        cert_wrapped_buf[0] = @intFromEnum(tls.HandshakeType.certificate);
                                        std.mem.writeInt(u24, cert_wrapped_buf[1..4], @intCast(cert_body_len), .big);
                                        cert_wrapped_buf[4] = cert_req_context_len;
                                        @memcpy(cert_wrapped_buf[5..][0..cert_req_context_len], cert_req_context_buf[0..cert_req_context_len]);
                                        const cert_list_len_off = 5 + @as(usize, cert_req_context_len);
                                        std.mem.writeInt(u24, cert_wrapped_buf[cert_list_len_off..][0..3], 0, .big);
                                        const cert_wrapped_len = cert_list_len_off + 3;
                                        const cert_wrapped = cert_wrapped_buf[0..cert_wrapped_len];

                                        p.transcript_hash.update(cert_wrapped);

                                        var cert_inner_buf: [cert_wrapped_buf.len + 1]u8 = undefined;
                                        @memcpy(cert_inner_buf[0..cert_wrapped_len], cert_wrapped);
                                        cert_inner_buf[cert_wrapped_len] = @intFromEnum(tls.ContentType.handshake);
                                        const cert_inner = cert_inner_buf[0 .. cert_wrapped_len + 1];

                                        const cert_nonce = nonce: {
                                            const V = @Vector(P.AEAD.nonce_length, u8);
                                            const pad = [1]u8{0} ** (P.AEAD.nonce_length - 8);
                                            const operand: V = pad ++ @as([8]u8, @bitCast(big(client_hs_write_seq)));
                                            break :nonce @as(V, pv.client_handshake_iv) ^ operand;
                                        };
                                        client_hs_write_seq += 1;

                                        cert_record_len = tls.record_header_len + cert_inner.len + P.AEAD.tag_length;
                                        cert_record_buf[0] = @intFromEnum(tls.ContentType.application_data);
                                        std.mem.writeInt(u16, cert_record_buf[1..3], @intFromEnum(tls.ProtocolVersion.tls_1_2), .big);
                                        std.mem.writeInt(u16, cert_record_buf[3..5], @intCast(cert_inner.len + P.AEAD.tag_length), .big);
                                        const cert_ad = cert_record_buf[0..tls.record_header_len];
                                        const cert_ciphertext = cert_record_buf[tls.record_header_len..][0..cert_inner.len];
                                        const cert_auth_tag = cert_record_buf[tls.record_header_len + cert_inner.len ..][0..P.AEAD.tag_length];
                                        P.AEAD.encrypt(cert_ciphertext, cert_auth_tag, cert_inner, cert_ad, cert_nonce, pv.client_handshake_key);
                                    }

                                    const client_finished_hash = if (client_cert_requested) p.transcript_hash.peek() else handshake_hash;
                                    ...
                                    // NOX PATCH: sabit `pv.client_handshake_iv` YERİNE (client_hs_write_seq
                                    // her zaman 0 İSE davranış AYNI kalır — okuma tarafının AYNI XOR
                                    // deseni), Certificate kaydı GÖNDERİLDİYSE seq=1 kullanılmalı (AYNI
                                    // anahtarla iki kaydı AYNI nonce'la şifrelemek AEAD güvenliğini KIRAR).
                                    const nonce = nonce: {
                                        const V = @Vector(P.AEAD.nonce_length, u8);
                                        const pad = [1]u8{0} ** (P.AEAD.nonce_length - 8);
                                        const operand: V = pad ++ @as([8]u8, @bitCast(big(client_hs_write_seq)));
                                    ...
                                    if (client_cert_requested) {
                                        var all_msgs_vec: [3][]const u8 = .{
                                            &client_change_cipher_spec_msg,
                                            cert_record_buf[0..cert_record_len],
                                            &finished_msg,
                                        };
                                        try output.writeVecAll(&all_msgs_vec);
                                    } else {
